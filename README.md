...
**交叉编译Windows产物**（Linux环境下通过Wine打包出.exe）：
```bash
# 构建镜像
docker build -f Dockerfile.windows -t m3u8-video-tool-win-builder .

# 运行构建，将产物挂载到本地dist目录
docker run --rm -v "$(pwd)/dist:/app/dist" m3u8-video-tool-win-builder
```
产物为 `dist/M3U8VideoTool.exe`
